...
const TicketCard = ({ ticket }) => {
  ...
  const getSeason = () => {
    const temporadaAttr = attributes.find(attr => attr.trait_type === "Validade");
    if (temporadaAttr) {
  ...
  const priceRealMock = (priceFanToken * 0.30).toFixed(2);
    ...
    >
      ...
      >
        <img
          ...
          onError={(e) => { e.target.onerror = null; e.target.src = '/vg.jpeg'; }}
        />
        ...
        <div className="absolute top-3 left-3 px-3 py-1 bg-black/60 backdrop-blur-sm rounded-full text-xs font-medium text-white border border-white/10">
          {type === 'comprar' ? 'Venda' : type === 'alugar' ? 'Aluguel' : 'Colecionável'}
        </div>
      ...
      <div className="p-4">
        ...
        <div className="flex items-start justify-between gap-3 mb-2">
          ...
          <img
            ...
            onError={(e) => { e.target.onerror = null; e.target.src = '/sp.png'; }}
          />
        </div>
